...
Key Generation:
    ...
    Also writes keys/public_key.hex, the value for -DEBLDR_PRODUCTION_KEY.
"""
...
def generate_keypair(output_dir: Path):
    ...
    # The one artefact the build consumes. This used to write public_key.h
    # defining ebldr_default_pubkey[] -- a symbol nothing includes or links,
    # so a developer who followed the quickstart built firmware whose trust
    # anchor was still the RFC 8032 test key, with no error to say so. The
    # bootloader takes its anchor from -DEBLDR_PRODUCTION_KEY=<64 hex> at
    # configure time (cmake/ProductionKey.cmake, docs/key_lifecycle.md), and
    # that is what is written here.
    write_public_key_hex(pub_raw, output_dir / "public_key.hex")


def write_public_key_hex(pub_raw: bytes, path: Path):
    """Write the raw Ed25519 public key as 64 lowercase hex characters -- the
    exact value EBLDR_PRODUCTION_KEY takes -- and print the configure flag."""
    hex_key = pub_raw.hex()
    assert len(hex_key) == 64, len(hex_key)
    path.write_text(hex_key + "\n")
    print(f"Public key (hex) saved: {path}")
    print(f"Public key (hex): {hex_key}")
    print("Build the bootloader with this key as its trust anchor:")
    print("  cmake -B build -DEBLDR_BOARD=<board> -DCMAKE_BUILD_TYPE=Release \\")
    print(f"        -DEBLDR_PRODUCTION_KEY=$(cat {path})")
    print("See docs/key_lifecycle.md; compare this value against the .pub before storing it.")
...
def extract_pubkey(key_path: Path, output_path: Path):
    ...
    write_public_key_hex(pub_raw, output_path)
...
def main():
    ...
    if args.extract_pubkey:
        ...
        output_path = Path(args.output) if args.output else Path('public_key.hex')
